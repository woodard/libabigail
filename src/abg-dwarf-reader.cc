      ...
      do
	{
	  ...
	  // Handle member types
		...
		build_ir_node_from_die(ctxt, &child, is_in_alt_di,
				       ...
				       /*is_member=*/true,
				       where_offset);
	      // Handle the access specifiers of the member type and
	      // its canonicalizing.
	      //
	      // Note that we are not interested in pointer, reference
	      // and qualified types that appear in the scope of this
	      // class because we are sticking them in the global
	      // space anyway.
	      //
	      // Different DWARF producers (e.g, gcc 4.4.x and 4.8.x)
	      // might put e.g a pointer type either in the global
	      // scope of in the scope of it's sub-type.
	      //
	      // This can lead to the comparison code thinking these
	      // types are different just because they have been
	      // generated by different DWARF producers.
	      if (td && (is_typedef(td)
			 || is_class_type(td)
			 || is_enum_type(td)))
		{
